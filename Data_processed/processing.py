...
t3 = driverstanding_df[driverstanding_df['raceId'].isin(t2)].sort_values(['raceId','points'],ascending=False)[['raceId','driverId','points','wins']]
t3['raceId'] = t3['raceId'].replace(t2.tolist(),t2.index.tolist())
t3 = t3.rename(columns={'raceId': 'year'}).merge(drivers_df,on='driverId',suffixes=['_x','']).sort_values(['year','points'],ascending=False)[['year','driverId', 'points', 'wins','forename','surname']]
...
t4 = constructorstanding_df[constructorstanding_df['raceId'].isin(t2)].sort_values(['raceId','points'],ascending=False)[['raceId','constructorId','points','wins']]
t4['raceId'] = t4['raceId'].replace(t2.tolist(),t2.index.tolist())
t4 = t4.rename(columns={'raceId': 'year'}).merge(constructors_df,on='constructorId',suffixes=['_x','']).sort_values(['year','points'],ascending=False)[['year','constructorId', 'points', 'wins','name']]
